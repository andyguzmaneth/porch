...
## Vector sets (to be generated from the reference implementation)

| Set | Directory | What it pins | Format |
|-----|-----------|--------------|--------|
| **Canonicalization** | `vectors/canonicalization/` | A raw JSON-RPC request → the exact canonical bytes on the wire. This is the anti-fingerprinting core (spec §6 Origin / §12 C2, a V2 property); two implementations MUST produce byte-identical output. | `{input, expected_canonical_hex}` |
...
Vectors are **not generated yet** — they come from the reference implementation (the EELS pattern: the runnable reference *is* the vector generator). This directory currently holds one hand-written seed vector to fix the format. A `speccheck`-style validator (cf. execution-apis) will check implementations against these.
